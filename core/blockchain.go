...
import (
	...
	"github.com/elastos/Elastos.ELA.SideChain.ESC/core/history"
	"github.com/elastos/Elastos.ELA.SideChain.ESC/core/rawdb"
	"github.com/elastos/Elastos.ELA.SideChain.ESC/core/state"
	"github.com/elastos/Elastos.ELA.SideChain.ESC/core/state/snapshot"
	...
	"github.com/elastos/Elastos.ELA.SideChain.ESC/internal/syncx"
	...
	"github.com/elastos/Elastos.ELA.SideChain.ESC/triedb/hashdb"
	"github.com/elastos/Elastos.ELA.SideChain.ESC/triedb/pathdb"
)
...
// and Processor.
func NewBlockChain(db ethdb.Database, genesis *Genesis, engine consensus.Engine, pbftEngine consensus.Engine, cfg *BlockChainConfig, shouldPreserve func(header *types.Header) bool) (*BlockChain, error) {
	if cfg == nil {
		cfg = DefaultConfig()
	}
	...
	if err != nil {
		log.Error("VerifyHeader failed", "error", err)
	}
...
func (bc *BlockChain) procFutureBlocks() {
	...
	if len(blocks) > 0 {
		...
		for i := range blocks {
			...
			if err != nil {
				log.Error("Failed to insert future block", "number", blocks[i].Number(), "hash", blocks[i].Hash(), "err", err)
			}
		}
	}
}
...
func (bc *BlockChain) writeBlockWithState(block *types.Block, receipts []*types.Receipt, statedb *state.StateDB) error {
	...
	if ptd == nil {
		return consensus.ErrUnknownAncestorTD
	}
...
func (bc *BlockChain) insertBlockChain(chain types.Blocks, setHead bool, makeWitness bool) (*stateless.Witness, int, error) {
	...
	witness, n, err := bc.insertChain(cliqueChain, setHead, makeWitness)
	if err != nil {
		return witness, n, err
	}
	pbftChain := chain[limit:]
	witness, n, err = bc.insertChain(pbftChain, setHead, makeWitness)
	return witness, n, err
}
...
func (bc *BlockChain) insertChain(chain types.Blocks, setHead bool, makeWitness bool) (*stateless.Witness, int, error) {
	...
	case errors.Is(err, consensus.ErrPrunedAncestor):
		if setHead {
			...
			log.Info("Pruned ancestor", "number", block.Number(), "hash", block.Hash())
...
func (bc *BlockChain) BadBlocks() []*types.Block {
	return rawdb.ReadAllBadBlocks(bc.db)
}
