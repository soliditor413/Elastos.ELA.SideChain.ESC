...
func (p *Pbft) OnRequestConsensus(id peer.PID, height uint64) {
	...
	status := p.dispatcher.HelpToRecoverAbnormal(id, height, p.chain.CurrentHeader().Number.Uint64())
	if status != nil {
...
func (p *Pbft) OnBlockReceived(id peer.PID, b *dmsg.BlockMsg, confirmed bool) {
	...
	currentBlockNumber := p.chain.CurrentBlock().Number
	blocks := types.Blocks{}
	blocks = append(blocks, block)
	log.Info("InsertChain", "height", block.GetHeight(), "block.NumberU64()-p.chain.CurrentBlock().NumberU64() ", block.Number().Cmp(currentBlockNumber), "currentBlock", currentBlockNumber.Uint64())

	if block.Number().Cmp(currentBlockNumber) >= 0 && block.NumberU64()-currentBlockNumber.Uint64() > 1 {
...
func (p *Pbft) OnSmallCroTxReceived(id peer.PID, msg *dmsg.SmallCroTx) {
	...
	height := p.chain.CurrentBlock().Number.Uint64()
