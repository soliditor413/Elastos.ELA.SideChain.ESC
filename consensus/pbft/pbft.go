...
import (
	...
	"github.com/elastos/Elastos.ELA.SideChain.ESC/dpos/crypto"
...
func (p *Pbft) subscribeEvent() {
	events.Subscribe(func(e *events.Event) {
		...
		case dpos.ETNewPeer:
			...
			if _, ok := p.chain.Engine().(*Pbft); ok && !p.dispatcher.GetConsensusView().HasProducerMajorityCount(count) {
...
func (p *Pbft) CurrentBlock() *types.Header {
	if p.chain == nil {
		return nil
	}
...
func (p *Pbft) VerifyHeader(chain consensus.ChainHeaderReader, header *types.Header, seal bool) error {
	dpos.Info("Pbft VerifyHeader")
	return p.verifyHeader(chain, header, nil, seal)
}

func (p *Pbft) VerifyHeaders(chain consensus.ChainHeaderReader, headers []*types.Header, seals []bool) (chan<- struct{}, <-chan error) {
...
func (p *Pbft) verifyHeader(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header, seal bool) error {
...
func (p *Pbft) VerifySeal(chain consensus.ChainHeaderReader, header *types.Header) error {
	dpos.Info("Pbft VerifySeal")
	return p.verifySeal(chain, header, nil)
}

func (p *Pbft) verifySeal(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
	...
	if oldHeader := chain.GetHeaderByNumber(number); oldHeader != nil {
		...
		if confirm.Proposal.ViewOffset == oldConfirm.Proposal.ViewOffset && oldHeader.Hash() != header.Hash() {
			log.Error("double sign block", "oldHeader.Hash()", oldHeader.Hash().String(), "header.Hash()", header.Hash().String(), "old.miner", oldHeader.Coinbase.String(), "newHeader.Miner", header.Coinbase.String(), "current best height", p.CurrentBlock().Number)
...
func (p *Pbft) Prepare(chain consensus.ChainHeaderReader, header *types.Header) error {
...
func (p *Pbft) Finalize(chain consensus.ChainHeaderReader, header *types.Header, statedb vm.StateDB, body *types.Body) {
...
func (p *Pbft) FinalizeAndAssemble(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, body *types.Body, receipts []*types.Receipt) (*types.Block, error) {
...
func (p *Pbft) Seal(chain consensus.ChainHeaderReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
...
func (p *Pbft) CalcDifficulty(chain consensus.ChainHeaderReader, time uint64, parent *types.Header) *big.Int {
...
func (p *Pbft) Start(headerTime uint64) {
	...
	if !p.enableViewLoop {
		...
		p.dispatcher.GetConsensusView().UpdateDutyIndex(p.chain.CurrentBlock().Number.Uint64())
		go p.changeViewLoop()
	} else {
