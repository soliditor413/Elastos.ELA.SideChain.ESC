// Copyright 2022 The Elastos ELA Side Chain ESC Authors
...
const ourPath = "github.com/elastos/Elastos.ELA.SideChain.ESC" // Path to our module
...
func versionInfo(info *debug.BuildInfo) string {
	// If the main package is from our repo, prefix version with "elastos-ela-sidechain".
	if strings.HasPrefix(info.Path, ourPath) {
		return fmt.Sprintf("elastos-ela-sidechain %s", info.Main.Version)
	}
	...
	if mod == nil {
		...
		// version.
		return version + fmt.Sprintf("elastos-ela-sidechain %s", WithMeta)
	}
