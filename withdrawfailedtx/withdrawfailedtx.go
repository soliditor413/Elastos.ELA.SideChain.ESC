...
func SendRefundTx(from common.Address, txid string) error {
	...
	if err != nil {
		log.Error("IpcClient PendingNonceAt is err:", "txid", txid, "err", err)
		return err
	}
	tx := types.NewTx(&types.LegacyTx{Nonce: nonce, To: &common.Address{}, Value: big.NewInt(0), Data: data, Gas: gasLimit, GasPrice: gasPrice})
	...
	hash := tx.Hash()
	log.Info("send refund tx", "txHash", hash.String(), "withdrawTx", txid, "gasPrice", gasPrice.Uint64(), "gasLimit", gasLimit)
	return nil
}
