...
func ProcessPledgedBill(elaTx it.Transaction) {
	...
	if createNft.GenesisBlockHash.String() != genesis.Hash().String() {
		log.Error("error genesis", "spv.GenesisHash", genesis.Hash().String(), "createNFT SideChain", createNft.GenesisBlockHash.String())
		return
	}
